<#
.SYNOPSIS
    Downloads and extracts .app files from a BC artifact ZIP using HTTP Range requests.

.DESCRIPTION
    Performs a HEAD request to verify Range support, then delegates to the Python
    download_range_helper.py for selective download and in-memory extraction.
    Returns the output path on success, or $null on failure (triggering fallback).

.PARAMETER ArtifactUrl
    Full URL to the BC artifact ZIP file.

.PARAMETER TargetFolderPrefix
    Folder prefix within the ZIP to extract (e.g., "Extensions/", "applications/").

.PARAMETER OutputPath
    Directory where extracted .app files will be written.
#>
param(
...
try {
    ...
    # Verify output has .app files
    $appFiles = Get-ChildItem -Path $OutputPath -Filter "*.app" -ErrorAction SilentlyContinue
    if (-not $appFiles -or $appFiles.Count -eq 0) {
        Write-Host "No .app files found in output directory"
        if (Test-Path $OutputPath) { Remove-Item -Recurse -Force $OutputPath }
        return $null
    }

    Write-Host "Range download complete: $($appFiles.Count) .app file(s) in $OutputPath"
    return $OutputPath
}
catch {
