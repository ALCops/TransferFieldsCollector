#!/usr/bin/env python3
"""Download and extract .app files from a ZIP archive using HTTP Range requests.
...
folder prefix, then extracts .app files from it in-memory. This avoids downloading
...
def main():
    ...
    try:
        ...
        # Step 3: Filter entries matching target prefix (.app files only, non-empty)
        matching = [
            e for e in entries
            if e['name'].startswith(target_prefix)
            and e['comp_size'] > 0
            and e['name'].lower().endswith('.app')
        ]
        print(f"Matching '{target_prefix}' .app files: {len(matching)} files")

        if not matching:
            prefixes = sorted(set(e['name'].split('/')[0] for e in entries if '/' in e['name']))
            print(f"Available folders: {prefixes}")
            print("ERROR: No matching .app entries found")
            sys.exit(1)
        ...
        # Step 6: Extract matching .app files in-memory
        extracted = 0
        ...
        print(f"Extracted {extracted} .app files to {output_dir}")

        if extracted == 0:
            print("ERROR: No .app files were extracted")
            sys.exit(1)
